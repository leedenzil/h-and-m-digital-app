...
export default function CheckoutPage() {
  ...
  // FIXED: Use getCartTotal instead of cartTotal
  const { cart, getCartTotal, removeFromCart, updateQuantity, clearCart } = useCart();
  const { user, isAuthenticated } = useContext(AuthContext);
  
  // FIXED: Helper function to calculate reward points
  const getCartRewardPoints = () => {
    return Math.round((getCartTotal() || 0) * 10);
  };
  ...
  // FIXED: Safe formatter function to prevent errors
          ...
          <>
            ...
            <Paper sx={{ p: 2, mb: 3 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                ...
                <Typography variant="body1">${formatPrice(getCartTotal() * 0.07)}</Typography>
              </Box>
              <Divider sx={{ my: 2 }} />
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                <Typography variant="h6">Total:</Typography>
                <Typography variant="h6">${formatPrice(getCartTotal() + (getCartTotal() * 0.07))}</Typography>
              </Box>
